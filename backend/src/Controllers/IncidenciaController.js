...
const { EmailAsignacion } = require("../utils/Emails");
...
async function crearIncidencias(req, res) {
    try {
        ...
        await IncidenciaAsignada.create({
            CT_cod_usuario: usuario,
            CT_id_incidencia: incidencias.CT_cod_incidencia,
        }); 
...
async function asignarIncidencias(req, res) {
    try {
        const { usuario, incidencia,riesgo,prioridad,afectacion,categoria } = req.body;
